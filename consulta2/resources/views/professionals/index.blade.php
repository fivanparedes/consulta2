@extends('layouts.app', ['activePage' => 'professional_index', 'title' => $companyName.' | Busca un profesional', 'navName'
                        ...
                        if (content.length > 0) {
                            ...
                            for (var i = 0; i < content.length; i++) {
                                ...
                                $('#listbody').append(
                                    ...
                                    .fullname +
                                    '</h4> <h5 class = "card-subtitle" ><span class = "badge badge-primary" >' +
                                    content[i].specialty + '</span> </h5> <p class = "card-text" >' +
                                    content[i].institution +
                                    '</p><p class="card-text"><strong>Cobertura médica: </strong>' +
                                    content[i].coverages + '</p> <a href = "/professional/show/' + content[
                                        i].id +
                                    '"class = "btn btn-primary" > Reservar turno</a> </div></div> </div> </div> </div>'
                                    );
                            ...
                            $('#listbody').empty();
                            $('#listbody').append(
                                '<div class="row"><div class="col-md-12"><div class="card"><div class="card-body">No hay registros que coincidan con los filtros.</div></div></div></div>'
                                );
                        }
                    }
                },
